...
namespace flyshark.utils.menu {
    ...
    export class MMenutImpl implements Menu {
         ...
         * @memberof MMenutImpl
         */
        public createMenu(menuList: any, menuHtmlElement: HTMLElement): void {
         ...
         * @memberof MMenutImpl
         */
        private getMenuHtml(menuList: any, iteration: any): string {
            let html = "";
            if (menuList != null && menuList.length > 0) {
                if (!iteration) {
                    html += "<ul class='site-menu'>";
                    let topMenu: Array<any> = [];
                    ...
                    //组装当前菜单的HTML
                    var menuClick: string = childMenuList.length > 0 ? "" : " onclick='toMenu(this.id)'  ";
                    ...
                    if (childMenuList.length > 0) {
                        html = `<li class='site-menu-item has-sub'><a href='javascript:void(0)' ${linkHtml}>${iconHtml}<span class='site-menu-title'>${iteration.menuTitle}</span><span class='site-menu-arrow'></span></a>`;
                    }
                    else {
                    ...
                    if (childMenuList.length > 0) {
                        ...
                        childMenuList.forEach((menu: any) => {
                            html += this.getMenuHtml(menuList, menu);
                        })
                        html += "</ul>";
                    }
