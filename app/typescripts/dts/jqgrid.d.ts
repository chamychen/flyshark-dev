...
interface TreeReader {
     ...
     * @memberof TreeReader
     */
    readonly sort_Controll_field: string;
...
interface JQuery {
     ...
     * @returns {} 
     */
    setGridParam(obj: any, overwrite: boolean): void;
    ...
    delRowData(rowid: string): void;

    setTreeNode(i: number, len: number): void;

    getGridRowById(rowid: string): HTMLElement;

    unformat(cellval: HTMLElement, options: any, pos: any, cnt: any): string;

    setGridWidth(newWidth: any, shrink: any): void;

    setGridHeight(newHeight: any): void;

    /**
     * 设置单元格的值或属性
     * 
     * @param {string} rowid 当前行id
     * @param {string} colname 列名称，也可以是列的位置索引，从0开始
     * @param {string} data 改变单元格的内容，如果为空则不更 新
     * @param {string} cssClass 如果是string则会使用addClass方法将其加入到单元格的css中，如果是array则会直接加到style属性中
     * @param {string} properties 设置单元格属性
     * @memberof JQuery
     */
    setCell(rowid: string, colname: string, data: string, cssClass: string, properties: string): void;

    disableEditCell(rowid: string, cellName: string): void;

    batchDisableEditCell(disableData: { rowId: string, cellNames: string[] }[]): void;
    ...
    disableEditRow(rowid: string): void;
    ...
    getCount(matchMetod: (rowData: any) => boolean, isOutStop: boolean): number;
    ...
    castToDefaultGrid(): void;

    resetTreeGrid(): void;

    getNodeByLongCode(longCode: string): any;

    getNodeChild(rowData: any, allLevel: boolean): any[]

    getNodeChildCount(rowData: any, allLevel: boolean): number;

    offsetNode(longCode: string, offsetValue: number): void;

    addTreeNode(rowData: any): void;
    ...
    move(rowId: string, newLongCode: string): void;

    movePosition(rowId: string, type: number): void;
